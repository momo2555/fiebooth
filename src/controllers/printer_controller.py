...
class PrinterController():
	...
	def print(self, image_path):
		...
		ret = conn.printFile(list(printers.keys())[0], tmp_img, "test print", {})
		print(ret)
		ret = conn.printFile(list(printers.keys())[1], tmp_img, "test print", {})
		print(ret)
		self.status()
		# # send print to printer
		# print_command = f"lp -d QL-800 {tmp_img}"
	...
	def status(self):
		...
		while printer_status != "PRINTING COMPLET":
			current_time = time.time()
			conn = cups.Connection()
			printers = conn.getPrinters()
